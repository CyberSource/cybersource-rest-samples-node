...
function EnrollWithPendingAuthentication(callback) {
	try {
	    ...
	    console.log('\n*************** EnrollWithPendingAuthentication ********************* ');
			...
			instance.checkPayerAuthEnrollment(request, function (error, data, response) {
				if (error) {
					console.log('\nError in EnrollWithPendingAuthentication : ' + JSON.stringify(error));
				}
				else if (data) {
					console.log('\nData of EnrollWithPendingAuthentication : ' + JSON.stringify(data));
				}
				console.log('\nResponse of EnrollWithPendingAuthentication : ' + JSON.stringify(response));
				console.log('\nResponse Code of EnrollWithPendingAuthentication : ' + JSON.stringify(response['status']));
				callback(error, data);
			});
		} catch (error) {
			console.log(error);
...
if (require.main === module) {
	EnrollWithPendingAuthentication(function () {
		console.log('\EnrollWithPendingAuthentication end.');
	}, false);
}
module.exports.EnrollWithPendingAuthentication = EnrollWithPendingAuthentication;