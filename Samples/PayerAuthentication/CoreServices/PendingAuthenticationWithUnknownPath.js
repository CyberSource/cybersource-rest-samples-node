...
function PendingAuthenticationWithUnknownPath(callback) {
	try {
	    ...
	    console.log('\n*************** PendingAuthenticationWithUnknownPath ********************* ');
			...
			instance.checkPayerAuthEnrollment(request, function (error, data, response) {
				if (error) {
					console.log('\nError in PendingAuthenticationWithUnknownPath : ' + JSON.stringify(error));
				}
				else if (data) {
					console.log('\nData of PendingAuthenticationWithUnknownPath : ' + JSON.stringify(data));
				}
				console.log('\nResponse of PendingAuthenticationWithUnknownPath : ' + JSON.stringify(response));
				console.log('\nResponse Code of PendingAuthenticationWithUnknownPath : ' + JSON.stringify(response['status']));
				callback(error, data);
			});
		} catch (error) {
			console.log(error);
...
if (require.main === module) {
	PendingAuthenticationWithUnknownPath(function () {
		console.log('\PendingAuthenticationWithUnknownPath end.');
	}, false);
}
module.exports.PendingAuthenticationWithUnknownPath = PendingAuthenticationWithUnknownPath;